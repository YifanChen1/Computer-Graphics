...
glm::vec3 raytraceSingleRay(Scene const &scene, Ray const &ray, int level, int source_id) {
	...
	glm::vec3 specular = phong.Is();
	glm::vec3 diffuse = phong.Id();
	glm::vec3 ambient = phong.Ia();
	glm::vec3 lighting = phong.I();

	Ray shadowRay = Ray(result.point, glm::normalize(scene.lightPosition - result.point));
	...
	if (level > 0) {
		Ray reflectRay = Ray(result.point, reflect(phong.ray.direction, result.normal));
		r = raytraceSingleRay(scene, reflectRay, level - 1, result.id);
	}


	return lighting + r * phong.material.reflectionStrength;
}
